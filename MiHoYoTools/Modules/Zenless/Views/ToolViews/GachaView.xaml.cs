...
using MiHoYoTools.Modules.Zenless.Views.GachaViews;
...
namespace MiHoYoTools.Modules.Zenless.Views.ToolViews
{
    ...
    public sealed partial class GachaView : Page
    {
        ...
        private void ReloadGachaMainView()
        {
            ...
            CreateCapture.IsEnabled = !string.IsNullOrEmpty(selectedUid);
        ...
        {
            string recordsBasePath = AppPaths.ResolveGameFolder(GameType.ZenlessZoneZero, "GachaRecords");
            DateTime now = DateTime.Now;
            string formattedDate = now.ToString("yyyy_MM_dd_HH_mm_ss");

            var suggestFileName = $"ZenlessTools_Gacha_Export_{selectedUid}_{formattedDate}_UIGF4";
        ...
        private void ClearGacha_Run()
        {
            try
            {
                ...
                GachaRepository.DeleteByGameUid(GameType.ZenlessZoneZero, selectedUid);
            ...
            if (ProxyButton.IsChecked ?? false)
            {
                if (Process.GetProcessesByName("ZenlessZoneZero").Length > 0)
                {
                    ...
                    try
                    {
                        ...
                        string folderPath = AppPaths.ResolveGameFolder(GameType.ZenlessZoneZero, string.Empty);
        ...
        public async void CreateCapture_Click(object sender, RoutedEventArgs e)
        {
            ...
            var checkBoxShowRecords = new CheckBox { Content = "是否显示调频记录", IsChecked = true };
