...
namespace MiHoYoTools.Modules.Zenless.Views.FirstRunViews
{
    public sealed partial class FirstRunGetDepend : Page
    {
        ...
        private async void OnGetDependLatestReleaseInfo()
        {
            ...
            try
            {
                string localZip = HelperSources.TryGetLocalZip(ZipFileName);
                if (!string.IsNullOrEmpty(localZip))
                {
                    fileUrl = localZip;
                    dispatcher.TryEnqueue(() =>
                    {
                        depend_Latest_Name.Text = "依赖项: 本地包";
                        depend_Latest_Version.Text = $"文件: {Path.GetFileName(localZip)}";
                ...
                string assetPrefix = Path.GetFileNameWithoutExtension(ZipFileName);
                var latestReleaseInfo = await _getGithubLatest.GetLatestDependReleaseInfoAsync(
