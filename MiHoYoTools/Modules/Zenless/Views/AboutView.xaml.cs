...
namespace MiHoYoTools.Modules.Zenless.Views
{
    public sealed partial class AboutView : Page
    {
        ...
        private async void AboutView_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            appVersion.Text = $"ZenlessTools {AppInfoHelper.GetVersionString()}";
            GetVersionButton();
            CheckFont();
            LoadSettings();
        ...
        public void Clear_AllData(TeachingTip sender, object args)
        {
            ...
            DeleteFolder(userDocumentsFolderPath + "\\JSG-LLC\\ZenlessTools\\", "1");
        }

        private void Clear_AllData_NoClose(object sender, RoutedEventArgs e, string Close = "0")
        {
            string userDocumentsFolderPath = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments);
            DeleteFolder(userDocumentsFolderPath + "\\JSG-LLC\\ZenlessTools\\", Close);
        }
        ...
        private async void Check_Update(object sender, RoutedEventArgs e)
        {
            ...
            var result = await GetUpdate.GetZenlessToolsUpdate();
            ...
            UpdateTip.Subtitle = isShiftPressed ? "尝试重装ZenlessTools" : status == 1 ? "新版本:" + result.Version : null;
        ...
        private async void Check_Depend_Update(object sender, RoutedEventArgs e)
        {
            ...
            UpdateTip.Title = isShiftPressed ? "遇到麻烦了吗" : status == 0 ? "无可用更新" : status == 1 ? "有可用更新" : "网络连接失败，可能是请求次数过多";
            UpdateTip.Subtitle = isShiftPressed ? "尝试重装ZenlessToolsHelper" : status == 1 ? "新版本:" + result.Version : null;
            UpdateTip.ActionButtonContent = isShiftPressed ? "强制重装" : status == 1 ? "查看详情" : null;
            UpdateTip.CloseButtonContent = "关闭";
        ...
        {
            bool isZenlessTools = UpdateTip.Target != checkDependUpdate;
            UpdateResult updateinfo = isZenlessTools ? await GetUpdate.GetZenlessToolsUpdate() : await GetUpdate.GetDependUpdate();
            UpdateTip.IsOpen = false;
            ContentDialog updateDialog = new ContentDialog
            {
                Title = (isZenlessTools ? "ZenlessTools" : "Helper") + ":" + updateinfo.Version + "版本可用",
            ...
            {
                if (isZenlessTools) StartUpdate(); else StartDependUpdate();
            }
        }
        ...
        private async void Install_Font_Click(object sender, RoutedEventArgs e)
        {
            ...
            installSFF.Content = "安装字体后重启ZenlessTools即生效";
