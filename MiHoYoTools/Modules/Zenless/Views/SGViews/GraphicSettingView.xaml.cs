...
namespace MiHoYoTools.Modules.Zenless.Views.SGViews
{
    public sealed partial class GraphicSettingView : Page
    {
        ...
        private async Task LoadGraphicsData(bool isForce)
        {
            ...
            if (isForce)
            {
                output = await ProcessRun.ZenlessToolsHelperAsync($"/GetReg {StartGameView.GameRegion}");
        ...
        private async void ChangeGraphic(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                if (settingsMap.ContainsKey(tag) && settingsMap[tag].ContainsKey(text))
                {
                    ...
                    await ProcessRun.ZenlessToolsHelperAsync($"/SetValue {StartGameView.GameRegion} {tag} {value}");
                }
                else if (booleanSettingsMap.ContainsKey(tag) && booleanSettingsMap[tag].ContainsKey(text))
                {
                    bool value = booleanSettingsMap[tag][text];
                    await ProcessRun.ZenlessToolsHelperAsync($"/SetValue {StartGameView.GameRegion} {tag} {value.ToString().ToLower()}");
                }
                else if (double.TryParse(text, out double scaleValue) && tag == "RenderScale")
                {
                    await ProcessRun.ZenlessToolsHelperAsync($"/SetValue {StartGameView.GameRegion} {tag} {scaleValue}");
                }
                else if (text.Contains("30") || text.Contains("60") || text.Contains("120"))
                {
                    await ProcessRun.ZenlessToolsHelperAsync($"/SetValue FPS {StartGameView.GameRegion} {text}");
                }
