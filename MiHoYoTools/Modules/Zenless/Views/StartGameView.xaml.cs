...
using MiHoYoTools.Modules.Zenless.Views.SGViews;
    ...
    public sealed partial class StartGameView : Page
    {
        ...
        private async void SelectGame(object sender, RoutedEventArgs e)
        {
            await AnsiConsole.Status().StartAsync("等待选择文件...", async ctx =>
            {
                ...
                {
                    ValidGameFile.Subtitle = "选择正确的ZenlessZoneZero.exe\n通常位于[游戏根目录\\ZenlessZoneZero.exe]";
        ...
        private void CheckProcess_Game(DispatcherQueueTimer timer, object e)
        {
            if (Process.GetProcessesByName("ZenlessZoneZero").Length > 0)
            {
                ...
                Frame_GraphicSettingView_Launched_Disable_Title.Text = "绝区零正在运行";
                ...
                Frame_AccountView_Launched_Disable_Title.Text = "绝区零正在运行";
        ...
        private async Task GetPromptAsync()
        {
            try
            {
                ...
                string url = "https://zentools.jamsg.cn/ZenlessTools_Prompt";
