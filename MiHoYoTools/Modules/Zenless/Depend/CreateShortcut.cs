...
namespace MiHoYoTools.Modules.Zenless.Depend
{
    public class CreateShortcut
    {
        ...
        {
            string shortcutPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop), "MiHoYoTools_Zenless.url");
            using (StreamWriter writer = new StreamWriter(shortcutPath))
            {
                writer.WriteLine("[InternetShortcut]");
                writer.WriteLine("URL=mihoyotools:///zenless/startgame");
            }
            NotificationManager.RaiseNotification("Shortcut created", "Zenless shortcut has been created on your desktop.", Microsoft.UI.Xaml.Controls.InfoBarSeverity.Success, true, 2);
        }
    }
}
