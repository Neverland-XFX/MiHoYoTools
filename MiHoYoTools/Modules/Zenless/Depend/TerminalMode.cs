...
namespace MiHoYoTools.Modules.Zenless.Depend
{
    internal class TerminalMode
    {
        ...
        public async Task<bool> Init(int Mode = 0, int SafeMode = 0, String PanicMessage = "Null",String OtherMessage = null)
        {
            ...
            Console.Title = "ZenlessTools 𝑻𝒆𝒓𝒎𝒊𝒏𝒂𝒍";
            Console.Clear();
            if (SafeMode == 0) { 
                if (Mode == 1)
                {
                    var list = new[] { "选择游戏路径", "抽卡分析", "设置", "[red]退出Terminal模式[/]", "[bold red]退出ZenlessTools[/]", };
                    if (AppDataController.GetGamePath != null)
                    {
                        ...
                        {
                            list = new[] { "选择游戏路径", "[Cyan]显示主界面[/]", "[red]退出Terminal模式[/]", "[bold red]退出ZenlessTools[/]", };
                        }
                        else
                        {
                            list = new[] { "[bold green]开启游戏[/]", "[bold yellow]清除游戏路径[/]", "[Cyan]显示主界面[/]", "[red]退出Terminal模式[/]", "[bold red]退出ZenlessTools[/]" };
                        }
                    }
                    else
                    {
                    ...
                    var select = AnsiConsole.Prompt(
                            new SelectionPrompt<string>()
                                .Title("[bold green]ZenlessTools[/] Terminal模式")
                                .PageSize(10)
                                .AddChoices(list));
            ...
            {
                Console.Title = "ZenlessTools SafeMode";
                Console.Clear();
                Logging.Write("[red]错误问题：[/]" + PanicMessage,2);
                Logging.Write("其他信息：" + OtherMessage,2);
                var list = new[] { "[red]清空所有配置文件[/]", "[bold red]退出ZenlessTools[/]" };
                var select = AnsiConsole.Prompt(
                        new SelectionPrompt<string>()
                            .Title("\n[bold red]ZenlessTools 安全模式[/]")
                            .PageSize(10)
                            .AddChoices(list));
                switch (select)
                {
        ...
        private async void SelectGame()
        {
            ...
            Logging.Write("通常位于(游戏根目录\\Game\\ZenlessZoneZero.exe)", 0);
            await AnsiConsole.Status().StartAsync("等待选择文件...", async ctx =>
            {
                var file = await picker.PickSingleFileAsync();
                if (file == null) { fileselect = 1; }
                else if (file.Name == "ZenlessZoneZero.exe")
                {
            ...
            {
                Logging.Write("选择文件不正确，请确保是ZenlessZoneZero.exe\n等待3秒后重新选择", 2);
