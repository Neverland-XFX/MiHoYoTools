...
using WinRT.Interop;
using MiHoYoTools.Views.GachaViews;
...
namespace MiHoYoTools.Views.ToolViews
{
    ...
    public sealed partial class GachaView : Page
    {
        ...
        private async Task LoadUIDs()
        {
            ...
            try
            {
                string recordsDirectory = AppPaths.ResolveGameFolder(GameType.StarRail, "GachaRecords");
        ...
        {
            string recordsBasePath = AppPaths.ResolveGameFolder(GameType.StarRail, "GachaRecords");
        ...
        private void ClearGacha_Run()
        {
            try
            {
                ...
                GachaRepository.DeleteByGameUid(GameType.StarRail, selectedUid);
            ...
            if (ProxyButton.IsChecked ?? false)
            {
                if (Process.GetProcessesByName("StarRail").Length > 0)
                {
                    ...
                    try
                    {
                        ...
                        string folderPath = AppPaths.ResolveGameFolder(GameType.StarRail, string.Empty);
        ...
        public async Task UpdateGachaRecord_Run(string uid, string mode = "?")
        {
            ...
            if (mode == "?" || mode == "srgf_merge")
            {
                if (mode == "srgf_merge")
                {
                    baseDir = Path.Combine(AppPaths.ResolveGameFolder(GameType.StarRail, "GachaRecords"), "tmp", uid);
                    newFilePath = Path.Combine(AppPaths.ResolveGameFolder(GameType.StarRail, "GachaRecords"), "tmp", $"{uid}_Converted.json");
                }
                else
                {
                    baseDir = Path.Combine(AppPaths.ResolveGameFolder(GameType.StarRail, "GachaRecords"), uid);
                    newFilePath = Path.Combine(AppPaths.ResolveGameFolder(GameType.StarRail, "GachaRecords"), $"{uid}.json");
                }
                ...
                if (mode == "srgf_merge")
                {
                    ...
                    string exportConvertedFilePath = Path.Combine(AppPaths.ResolveGameFolder(GameType.StarRail, "GachaRecords"), "tmp", $"{uid}_Converted_Export.json");
                    await ExportGacha.ExportAsync(newFilePath, exportConvertedFilePath);
                    await ImportGacha.Import(exportConvertedFilePath);
                    Directory.Delete(Path.Combine(AppPaths.ResolveGameFolder(GameType.StarRail, "GachaRecords"), "tmp"), true);
                    ReloadGachaView();
                }
