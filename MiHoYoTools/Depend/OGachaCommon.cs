...
using System.IO;
using MiHoYoTools.Views.ToolViews;
...
namespace MiHoYoTools.Depend
{
    class OGachaCommon
    {
        ...
        public async Task<bool> ExportAll()
        {
            ...
            var gachaFolderRelative = AppPaths.ResolveGameFolderRelative(GameType.StarRail, Path.Combine("GachaRecords", GachaView.selectedUid));
            var srtoolsFolder = await KnownFolders.DocumentsLibrary.GetFolderAsync(gachaFolderRelative);
            ...
            OGachaCommon data = new OGachaCommon
            {
                info = new Info
                {
                    ...
                    export_app = "SRTools",
                    export_app_version = AppInfoHelper.GetVersionString(),
                    srgf_version = "v1.0"
                },
                list = items
            };
        ...
        public async Task ImportSRGF()
        {
            ...
            string srtoolsFolderPath = AppPaths.ResolveGameFolder(GameType.StarRail, Path.Combine("GachaRecords", "tmp", uid));
